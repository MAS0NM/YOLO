...
def plot_lr_scheduler(optimizer, scheduler, epochs=300, save_dir=''):
    ...
    for _ in range(epochs):
        ...
        y2.append(optimizer.param_groups[2]['lr'])
...
def plot_lr_scheduler_warmup(optimizer, scheduler, nb, epochs=300, save_dir=''):
    # Plot LR simulating training for full epochs
    optimizer, scheduler = copy(optimizer), copy(scheduler)  # do not modify originals
...
def plot_results(start=0, stop=0, bucket='', id=(), labels=(),
    ...
    if bucket:
        ...
        # for debug
        # files = glob.glob(str(Path('/home/wangxiaoyun/yolov5-2.0/yolov5-2.0/runs/exp37/results.txt')))
    for fi, f in enumerate(files):
        try:
            # results = np.loadtxt(f, usecols=[2, 3, 4, 8, 9, 12, 13, 14, 10, 11], ndmin=2).T
            results = np.loadtxt(f, usecols=[2, 3, 4, 9, 10, 13, 14, 15, 11, 12], comments='#', ndmin=2).T
            # for debug
            # print(results)
            n = results.shape[1]  # number of rows
            x = range(start, min(stop, n) if stop else n)
