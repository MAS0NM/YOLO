...
def train_func(hyp, tb_writer, opt, device, pruning_rate=[], nw_ref=0, warmup=True, savelog=True, model_ref=None, setting_epochs=0, 
    ...
    # losses = AverageMeter()
    model.train()
    mloss_bn = torch.zeros(4, device=device)  # mean losses
    with torch.no_grad():
        pbar = enumerate(dataloader)
        pbar = tqdm(pbar, total=nb)  # progress bar
    ...
    return ema.ema.module if hasattr(ema.ema, 'module') else ema.ema, results #, loss_items, 
    # return results
