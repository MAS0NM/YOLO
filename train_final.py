...
def train_finalc(hyp, tb_writer, opt, device, pruning_rate=[], nw_ref=0, warmup=True, savelog=True, model_ref=None, setting_epochs=0, 
                    dataloader=None, testloader=None, dataset=None):
    # print(f'Hyperparameters {hyp}')

    ##################################
    ### runing setting (log file) #####
    ##################################
    if savelog:
        ...
        pruning_results_file = log_dir + os.sep + 'pruning_results.txt'
    ...
    ##################################
    epochs = setting_epochs if setting_epochs else opt.epochs
    batch_size, total_batch_size, weights, rank = \
        opt.batch_size, opt.total_batch_size, opt.weights, opt.local_rank
    ...
    if rank in [-1, 0] and savelog:
        for f in glob.glob('*_batch*.jpg') + glob.glob(results_file):
            os.remove(f)

    if savelog:
        with open(results_file, 'w') as file:
            file.write('#\t\tEpoch   gpu_mem  GIoU_loss  obj _loss  cls_loss    total       targets   img_size      lr                P           R           mAP@.5      mAP@.5:.95    val/giou_loss  val/obj_loss  val/cls_loss\n')  # write results.txt
    ...
    if model_ref is None:
        ...
        for m in model.modules():
            if isinstance(m, nn.Conv2d) and m not in excluded_layers:
                ...
                pruning_idx += 1
                # print(pruning_plan)
                # with open(pruning_results_file, 'a') as file:
        ...
        # pruning_param = '\nFilter Pruning PARAMs = {:.2f}M | {:.2f}M | ({:.2f}X | {:.2f}%) \n'.format(
        #                     before_params / 1000000, 
        #                     after_params/1000000, 
        #                     float(before_params/after_params), 
        #                     float(1.0-after_params/before_params)*100)
    ...
    if warmup:
        ...
        # https://discuss.pytorch.org/t/a-problem-occured-when-resuming-an-optimizer/28822
        if savelog:
            plot_lr_scheduler_warmup(optimizer, scheduler, nb, epochs, log_dir)
    else:
        # Scheduler https://arxiv.org/pdf/1812.01187.pdf
        # lf = lambda x: (((1 + math.cos(x * math.pi / epochs)) / 2) ** 1.0) * 0.8 + 0.2  # cosine
        lf = lambda x: (((1 + math.cos(x * math.pi / (epochs*nb))) / 2) ** 1.0) * 0.8 + 0.2  # cosine
    ...
    # mapresults, mapss, timess = test.test(opt.data,
    ...
    # model.train()
    # with torch.no_grad():
    #     ptar = enumerate(dataloader)
    #     for batch, (inputs, targets, paths, _) in ptar:
    #         # if batch < 50:
    #             imgs = inputs.to(device, non_blocking=True).float() / 255.0
    #             pred = model(imgs)
    ...
    nw = nw_ref if nw_ref else max(3 * nb, 1e3)  # number of warmup iterations, max(3 epochs, 1k iterations)
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        for i, (imgs, targets, paths, _) in pbar:  # batch -------------------------------------------------------------
            ...
            if nw < len(dataloader) and ni>nw:
                break
            ...
            if rank in [-1, 0]:
                ...
                # best_loss = min(best_loss, mloss[3])
                # print(best_loss)
                # print(best_loss.cpu().numpy())
        ...
        if rank in [-1, 0]:
            ...
            if not opt.notest or final_epoch:  # Calculate mAP
                savedir = log_dir if savelog else ''
    ...
    return ema.ema.module if hasattr(ema, 'module') else ema.ema, best_acc, results # best_loss
    # return results
...
if __name__ == '__main__':
    ...
    if not opt.evolve:
        ...
        train_func(hyp, tb_writer, opt, device, warmup=True, savelog=False)
