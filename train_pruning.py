...
def train(hyp, tb_writer, opt, device):
    ...
    # pruning_rate = [i*2 for i in pruning_rate]

    # from read_write import write_pruning_rate
    write_pruning_rate(pruning_results_file, pruning_rate)
    ...
    #     print(len(prunable_modules))
    #     print(len(pruning_rates))
    ...
    # Get macs and params before pruned according to thop
    from thop import profile
    input = torch.randn(1, 3, opt.img_size[0], opt.img_size[0])
    before_macs, before_params = profile(model, inputs=(input, ))
    ...
    for m in model.modules():
        if isinstance(m, nn.Conv2d) and m not in excluded_layers:
            ...
            pruning_plan.exec()
    # filter_prune_L1(model, torch.randn(1, 3, imgsz, imgsz), output_transform=None, pruning_rates=pruning_rate)

    # Get params according to tp.utiles
    # num_params_after_pruning = tp.utils.count_params( model )
    # pruning_params = "\nParams: %s => %s"%( num_params_before_pruning, num_params_after_pruning)

    # Get macs and params after pruned according to thop
    after_macs, after_params = profile(model, inputs=(input, ))
    pruning_flops = '\nFilter Pruning FLOPs = {:.2f}M | {:.2f}M | ({:.2f}X | {:.2f}%) '.format(
                        before_macs / 1000000, 
                        after_macs/1000000, 
                        float(before_macs/after_macs), 
                        float(1.0-after_macs/before_macs)*100)
    ...
    # pruning_s = pruning_params + pruning_flops + pruning_param
    pruning_s = pruning_flops + pruning_param
    with open(pruning_results_file, 'a') as file:
        file.write(pruning_s)
