...
def test_candidates_model(model_init, imgsz, candidates_pruning_rates, trainer, device, total_flops, total_params):

    acc_total_candidates = []
    # loss_total_candidates = []
    cnt = 0
    for cur_pruning_rate in candidates_pruning_rates:
        print('\n>>>>>> the {}th model'.format(cnt), flush=True)

        print('the current Filter Prune Rate : ')
        print(cur_pruning_rate[:-2])
        # print(t_current_prune_fp)
        print('\nFLOPs = {:.2f}M ({:.2f}X | {:.2f}%) '.format(cur_pruning_rate[-1]/1000000, 
                            float(total_flops/cur_pruning_rate[-1]), 
                            float(1.0-cur_pruning_rate[-1]/total_flops)*100), flush=True)
        print('PARAMs = {:.2f}M ({:.2f}X | {:.2f}% ) '.format(cur_pruning_rate[-2]/1000000, 
                            float(total_params/cur_pruning_rate[-2]), 
                            float(1.0-cur_pruning_rate[-2]/total_params)*100), flush=True, end='\n\n')
        ...
        model, _, results = trainer.train(setting_epochs=1, in_nw=80, in_model=model, warmup=True)
    ...
    for candidator in acc_total_candidates:
        print('Filter Pruning FLOPs = {:.2f}M ({:.2f}X | {:.2f}%) |'.format(candidator[0][-1]/1000000,
                    float(total_flops/candidator[0][-1]),
                    float(1.0-candidator[0][-1]/total_flops)*100), flush=True)
        print('PARAMs = {:.2f}M ({:.2f}X | {:.2f}% )|'.format(candidator[0][-2]/1000000,
                    float(total_params/candidator[0][-2]),
                    float(1.0-candidator[0][-2]/total_params)*100), flush=True)
        print('Score = {:.4f}   '.format(candidator[1]))
    print('---------------------------------------------------------')

    return acc_total_candidates


def search_filter_pruning(hyp, tb_writer, opt, device):
    ...
    for epoch in range(start_epoch, search_epochs):
        if epoch == 1:

            candidates_prune_fps = random_can_fp(model_init, max_FLOPs_FP, max_PARAMs_FP, target_params_fp, target_flops_fp, 
                imgsz, init_group_num, FP_Rate_Len, total_flops, total_params)

            acc_total_candidates = test_candidates_model(model_init, imgsz, candidates_prune_fps, trainer, device, total_flops, total_params)
            
            global_candidate = acc_total_candidates
            
            acc_candidates, acc_candidates_score = select_global_candidate(global_candidate, 25)
            ...
            prune_mutation_fp = get_mutation_fp(model_init, max_FLOPs_FP, max_PARAMs_FP, target_params_fp, target_flops_fp, 
                    imgsz, mutation_epoch, acc_candidates, acc_candidates_score, FP_Rate_Len, 10, 0.1, 4, total_flops, total_params)

            prune_crossover_fp = get_crossover_fp(model_init, max_FLOPs_FP, max_PARAMs_FP, target_params_fp, target_flops_fp, 
                        imgsz, acc_candidates, acc_candidates_score, FP_Rate_Len, 10, total_flops, total_params)

            candidates_prune_fp = []
            candidates_prune_fp.extend(prune_mutation_fp)
            candidates_prune_fp.extend(prune_crossover_fp)
            total_candidates = test_candidates_model( model_init, imgsz, candidates_prune_fp, trainer, device, total_flops, total_params)
        ...
        # summary
        best_candidates_prune = [can for can in acc_candidates[0][:-2]]
        print("best_candidates_prune="+str(best_candidates_prune))
        print('Filter Pruning FLOPs = {:.2f}M ({:.2f}X | {:.2f}% ) |'.format(acc_candidates[0][-1]/1000000, 
                    float(total_flops/acc_candidates[0][-1]), 
                    float(1.0-acc_candidates[0][-1]/total_flops)*100), flush=True, end=' ')
        print('PARAMs = {:.2f}M ({:.2f}X | {:.2f}% ) |'.format(acc_candidates[0][-2]/1000000, 
                    float(total_params/acc_candidates[0][-2]), 
                    float(1.0-acc_candidates[0][-2]/total_params)*100), flush=True, end=' ')
        print('Top1 = {:.2f}%'.format(acc_candidates_score[0]))
...
if __name__ == '__main__':
    ...
    search_filter_pruning(hyp, tb_writer, opt, device)
