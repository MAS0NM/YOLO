...
def test(data,
    ...
    if training:  # called by train.py
        ...
        # s_total_flops = '\nModel FLOPs = {:.2f}M'.format(before_macs / 1000000)
        # s_total_params = 'Model PARAMs = {:.2f}M\n'.format(before_params / 1000000)
        # print(s_total_flops)
        # print(s_total_params)
        ...
        # # #################################################
        # # # test this part of code when training=false 
        # # #################################################
        # # #################################################
        # # # Torch Pruning (Begin)  
        # # #################################################
        ...
        #         pruning_plan = DG.get_pruning_plan( m, tp.prune_conv, idxs=strategy(m.weight, amount=0.1) )
        #         print("pruning plan ")
        #         print(pruning_plan)
    ...
    s = ('%20s' + '%12s' * 9) % ('Class', 'Images', 'Targets', 'P', 'R', 'mAP@.5', 'mAP@.5:.95', 'loss_GIoU', 'loss_obj', 'loss_cls')
    ...
    for batch_i, (img, targets, paths, shapes) in enumerate(tqdm(dataloader, desc=s)):
        ...
        with torch.no_grad():
            ...
            # from thop import profile
            # # input = torch.randn(1, 3, 320, 320).to(device)
            # before_macs, before_params = profile(model, inputs=(img, ))
            # s_total_flops = '\nModel FLOPs = {:.2f}M'.format(before_macs / 1000000)
            # s_total_params = 'Model PARAMs = {:.2f}M\n'.format(before_params / 1000000)
            # print(s_total_flops)
            # print(s_total_params)
        ...
        for si, pred in enumerate(output):
            ...
            # print('correct ', correct.cpu())
            # print('id ', pred[:, 4].cpu())
            # print('score ', pred[:, 5].cpu())
            # print('tcls ', tcls)
            # print("-----------------------")
    ...
    print(pf % ('all', seen, nt.sum(), mp, mr, map50, map, *(loss.cpu() / len(dataloader)).tolist()))
...
# python test.py --weights ./weights/DET-biaopan_quexian-20211028.pt --batch-size 32 --data data/quexian.yaml --img-size 320 --conf-thres 0.3 --iou-thres 0.5 --device 0
