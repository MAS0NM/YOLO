...
class DependencyGraph(object):
    ...
    for t1 in HANDLER.keys():
        for t2 in HANDLER.keys():
            # print("t1  t2  ", t1,t2)
            # print("HANDLER[t1][0]  ", HANDLER[t1][0])
            # print("HANDLER[t2][1]  ", HANDLER[t2][1])
            # print("HANDLER[t1][1]  ", HANDLER[t1][1])
            # print("HANDLER[t2][0]  ", HANDLER[t2][0])
    ...
    CUSTOMIZED_OP_FN = {}
    # print(OUTPUT_NODE_RULES[0])
    # print(INPUT_NODE_RULES[0])
    ...
    def build_dependency( self, 
        ...
        # print("module_to_name  ", self._module_to_name)
        tmp_module_to_name = { name for (name, module) in model.named_modules() }
        # print("module_to_name  ", tmp_module_to_name)
        # print("pruning_dim  ", pruning_dim)
        # print("example_inputs.size()  ", example_inputs.size())
        if pruning_dim >= 0:
        ...
        # print("self.module_to_node  ", self.module_to_node)
    ...
    def _build_dependency(self, module_to_node):
        for module, node in module_to_node.items():
            ...
            for in_node in node.inputs:
                # print("(node.type, in_node.type)  ", node.type, in_node.type)
                in_node_rule = self.INPUT_NODE_RULES.get( (node.type, in_node.type), None )
                # print("in_node_rule  ", in_node_rule)
            ...
            for out_node in node.outputs:
                # print("(node.type, out_node.type)  ", node.type, out_node.type)
                out_node_rule = self.OUTPUT_NODE_RULES.get( (node.type, out_node.type), None )
                # print("out_node_rule  ", out_node_rule)
    ...
    def _obtain_forward_graph(self, model, example_inputs, output_transform, pruning_dim):
        ...
        # print("grad_fn_to_module  ", grad_fn_to_module)
        ...
        def _build_graph(grad_fn):
            ...
            if module is not None and module in module_to_node and module not in reused:
                # print('module is not None and module in module_to_node and module not in reused')
                return module_to_node[module]
            
            if module is None:
                # print('module is None')
                if not hasattr(grad_fn, 'name'):
                    # print('hasattr(grad_fn, name)', hasattr(grad_fn, 'name'))
                    ...
                    # print('module = _ConcatOp()')
                    module = _ConcatOp()
                elif 'splitbackward' in grad_fn.name().lower():
                    # print('module = _SplitOp()')
                    module = _SplitOp()
                else:
                    # print('module = _ElementWiseOp()')
            ...
            if module not in module_to_node:
                ...
                # print('grad_fn', grad_fn)
                # print('pruning_dim', pruning_dim)
                # print('self._module_to_name.get( module, None )', self._module_to_name.get( module, None ))
                # print('self.CUSTOMIZED_OP_FN', self.CUSTOMIZED_OP_FN)
